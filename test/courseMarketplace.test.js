...
//   const courseId = web3.utils.randomHex(16);
//   const proof = web3.utils.randomHex(32);
...
//     it("shouldn't be able to purchase the same course twice", async () => {
//       await catchRevert(
//         _contract.purchaseCourse(courseId, proof, { from: buyer, value })
//       );
//     });

//     it("can get the purhcased course hash by index", async () => {
//       const index = 0;
...
//       const beforeTxBuyerBalancer = await getBalance(buyer);
//       const beforeTxContractBalance = await getBalance(_contract.address);
...
//       const afterTxBuyerBalancer = await getBalance(buyer);
//       const afterTxContractBalance = await getBalance(_contract.address);
...
//       assert.equal(
//         toBN(beforeTxBuyerBalancer).sub(toBN(value)).sub(gas).toString(),
//         afterTxBuyerBalancer,
...
//   // Add more tests for other functions in the CourseMarketplace contract
// });
